            ...
            <table class="tbl_view01" summary="검색조건 테이블입니다. 항목으로는 등이 있습니다">
                                            ...
                                            <c:choose>
                                               ...
                                               <c:when test="${smssendlist.sendType eq 'marketing'}">
                                                <c:if test="${smssendlist.censorId eq null}">
                                                    <li class="disib mgl20" id="validation-num-li"><b>심의필번호</b><span class="th_must">*</span>
                ...
                <tr <c:if test="${smssendlist.msgDstic eq 'SM'|| smssendlist.msgDstic eq 'LM'}">style='display:none'</c:if> id="img-tr">
                    ...
                    <td>
                        <ul class="list01">
                            <li>
                                <input type="text" id="image-fileupload-input1" placeholder="파일 업로드" value="${smssendlist.imagePath1}"
                                       style="width:330px">
                            </li>
                            <li>
                                <input type="text" id="image-fileupload-input2" placeholder="파일 업로드" value="${smssendlist.imagePath2}"
                                       style="width:330px">
                            </li>
                            <li>
                                <input type="text" id="image-fileupload-input3" placeholder="파일 업로드" value="${smssendlist.imagePath3}"
                                       style="width:330px">
                            </li>
                        </ul>
                    </td>
                        ...
                        <div class="faq">
                            <ul class="faqBody">
                                <li class="article hide" id="a4"  >
                                    ...
                                    <span class="ment_sms01" style='font-weight:normal;'> &nbsp; * 최소 2명 ~ 최대 5명까지 가능(본인 포함) </span>
                                    </p>
                                    <div class="a" style="display: none;">
                                    <!-- table_view -->
...
//              remainCount();
  ...
  $("#employee-search-btn").on('click', function (e) {
    if($('#send-emp-table').find('tr').length == 5){
          showAlert("발송승인 직원은 최대 5명까지 가능합니다.");
          return false;
    }
  ...
  $("#confirm_in_search-btn").on('click', function () {
     ...
     // 승인자 추가시 넘버링 오류 수정 @jys
      ...
      //메시지 제목 및 내용 확인
        ...
        if($('#tr_confirm-doc-name').is(":visible")){
          ...
          if($('#send-emp-table').find('tr').length <= 1 || $('#send-emp-table').find('tr').length > 5){
                showAlert("발송승인 직원은 최소 2명~최대 5명까지 가능합니다.");
                return false;
          }
		  ...
		  $.ajax({
		    ...
		    showAlert("저장에 실패하였습니다. 관리자에게 문의하세요.");
		  });
          ...
          $.ajax({
              ...
              showAlert("발송승인 요청에 실패하였습니다. 관리자에게 문의하세요.");
            });
      ...
      function saveReqData(){
       ...
       $(".article").each(function (){
            ...
            $(this).find(".confirm-emp-id").each(function (index) {
                sendConfirmEmp.push({
                  emplId : $(this).val()
                  ,emplHpNo : $('.sendConfirm-emp-emplHpNo').val()
                  ,agreeType : type
              });
            });
       });
     ...
     // MMS 이미지 파일 갯수 체크
        var imageCount = null;
        if('${smssendlist.msgDstic}' == "mms"){
            imageCount = 0;
            if($("#real-image-fileupload-input1").val() != null){
                imageCount + 1; 
            }
            if($("#real-image-fileupload-input2").val() != null){
                imageCount + 1; 
            }
            if($("#real-image-fileupload-input3").val() != null){
                imageCount + 1; 
            }
        }
    ...
    if(changeHeadDataYn == "N" && $("#msg-type").val().toUpperCase() == "MMS"){
        ...
        var sendTime = '';
        var tempDate = '';
        var t21_pengagYms = "99991231235959";
         
        // 발송예정시간 및 건수를 request param 으로 만드는 작업
        var sendDateCount = [];
        
        $(".uneditable-select").each(function (index) {
        ...
        var confirmBodyMessage = 
            ...
            + ' 총 발송건수 : ' + commaReqNum + '\n'
            + ' 발송 일정 : ' + sendTime + ' \n'
            + ' 승인 URL : ' + 'http://localhost:9090/ibk/smssendlist/confirm/' + groupUniqNo +'/' ;
                ...
                data: JSON.stringify({
                    ...
                    "timeList":timeList}),
                        ...
                        $.each(data, function(key, value){
                            ...
                            for(var i=0; i< timeList.length; i++){
                            var num = parseInt(value["TOTAL_TIME_"+ timeList[i]]);
                                $("#sel-" + key + "-" + timeList[i]).val($.number(value["TOTAL_TIME_"+ timeList[i]]));
                            }
