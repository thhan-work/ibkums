...
<div id="content">
    ...
    <div class="box_search01">
      <ul>
        <li>
			...
			<span>
				<select style="width:150px;" id="search-word-type">
					<option value="group_code" selected>공통그룹코드</option>
					<option value="group_code_name">공통그룹코드명</option>
				</select>
			</span>
			<span>
				<input type="text" maxlength="100" value="" id="search-word" style="width:150px">
			</span>
			
			<span class="title02">활성여부</span>
			<span>
    ...
    <div class="tbl_wrap_list">
        <table class="tbl_list" id="code_group_table" summary="공통코드 그룹 테이블 입니다.">
            <colgroup>
                <col style="width:"/>
                <col style="width:"/>
                <col style="width:"/>
...
<!-- codegroup popup -->
<jsp:include page="manage-codegroup-popup.jsp" flush="true"/>
...
<script>
  ...
  $(document).ready(function () {
    ...
    IbkDataTable.initDataTable("code_group_table", "./manage-codegroup", requestParam,
    		IbkCodeGroupColumn.columns, IbkCodeGroupColumn.columnDefs);
  });
  ...
  // datatable 수정 버튼 클릭 이벤트 리스너
   $("#code_group_table").on('click', '.mod_btn', function () {
	   modification_init();
       $.ajax({
           url: '${pageContext.request.contextPath}/manage-codegroup/' + $(this).prop('id').split("-")[1],
           type: 'GET',
           dataType: 'JSON',
           success: function (result) {
        	   setCodeGroupInfo($("#popup_modify"), result);
           }
       });
   });
  ...
  function deleteCodeGroup(deleteTarget) {
    if (deleteTarget.length > 0) {
      $.ajax({
        url: '${pageContext.request.contextPath}/manage-codegroup/' + deleteTarget,
        type: 'DELETE',
        success: function (result) {
          showAlert("삭제 하였습니다.");
          IbkDataTable.reload();
